...
const FormSelect: FC<Props> = ({ label, name, errors, control, icon: Icon, onChange }) => {
  ...
  const { data: users, error: errorUser } = useSWR<User[]>("/api/user?type=prof", fetcher);

  const selectOptions = users?.map((v) => ({
    label: v.firstName + " " + v.lastName + " (" + v.role + ")",
    value: v,
  }));
        ...
        <Controller
          ...
          render={({
            ...
            <Select
              ...
              oOptionsMessage={() => "Pas de prof"}
              isLoading={!users && !errorUser}
            />
          )}
        />
      </div>
